function [U,B_k,V,p,ierr,work] = lanbpro_modified_nocomplex(varargin)
...
if isempty(U)
    ...
    int = [1:j]';
    [p,beta(j+1),rr] = reorth_modified_mex(U,p,beta(j+1),int,gamma,cgs);
...
for j=j0:k
    ...
    if j==1
        ...
        if onesided~=-1 && ( fro || numax(j) > delta || force_reorth ) && alpha(j)~=0
            ...
            % Reorthogonalize v_j
            [r,alpha(j),rr] = reorth_modified_mex(V,r,alpha(j),int,gamma,cgs);
    ...
    if alpha(j) < max(n,m)*anorm*eps && j<k,
        ...
        for attempt=1:3
            ...
            int = [1:j-1]';
            [r,nrmnew,rr] = reorth_modified_mex(V,r,nrm,int,gamma,cgs);
    ...
    if beta(j+1) < max(m,n)*anorm*eps  && j<k,
        ...
        for attempt=1:3
            ...
            int = [1:j]';
            [p,nrmnew,rr] = reorth_modified_mex(U,p,nrm,int,gamma,cgs);
            npu = npu + rr*length(int(:));  nreorthu = nreorthu + 1;
            mu(int) = m2*eps;
            if nrmnew > 0
                % A vector numerically orthogonal to span(Q_k(:,1:j)) was found.
...
%       Stephen Becker says: don't use this, use matlab's "hypot"
