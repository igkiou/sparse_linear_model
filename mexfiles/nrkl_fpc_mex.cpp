...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	if ((nrhs >= 9) && (mxGetNumberOfElements(prhs[8]) != 0)) {
		tauMultiplier = *(DOUBLE*) mxGetData(prhs[8]);
	} else {
		tauMultiplier = 10000;
	}
	...
	if ((nrhs >= 10) && (mxGetNumberOfElements(prhs[9]) != 0)) {
		tauRate = *(DOUBLE*) mxGetData(prhs[9]);
	} else {
		tauRate = 0.9;
	}
	...
	if (numRepeats > 1) {
		nrkl_fpc(K, distLabelType, constraintMat, betaVec, weights, tau, delta, \
				numIters, tolerance, numPoints, numConstraints, numRepeats);
	} else {
		nrkl_fpc_continuation(K, distLabelType, constraintMat, betaVec, weights, \
				*tau, delta, numIters, tolerance, tauMultiplier, tauRate, \
