...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	if (nrhs != 9) {
		ERROR("Nine input arguments are required.");
    }
	...
	DOUBLE *DDt2 = (DOUBLE*) mxGetData(prhs[3]);
	DOUBLE *DDt3 = (DOUBLE*) mxGetData(prhs[4]);
	DOUBLE *VL = (DOUBLE*) mxGetData(prhs[5]);
	DOUBLE *L = (DOUBLE*) mxGetData(prhs[6]);
	DOUBLE alphaReg = *(DOUBLE*) mxGetData(prhs[7]);
	DOUBLE betaReg = *(DOUBLE*) mxGetData(prhs[8]);
	...
	} else if ((INT) mxGetNumberOfElements(prhs[6]) != N) {
	...
	eig_lsqr_obj_grad_smalldata(obj, deriv, Phi, X, Y, DDt2, DDt3, VL, L, alphaReg, betaReg, M, N, numSamples, derivFlag,\
