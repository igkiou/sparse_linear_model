...
if issparse(data)
	...
	if (~isfield(options, 'KeepMean') || options.KeepMean == 0),
		sampleMean = mean(data);
		% NOTE: Comment out removal of mean.
...
if ~isfield(options,'ReguDict') || isempty(options.ReguDict),
	[eigvector, elapse.timeReg] = SR(options, Y, data);
else
	[eigvector, elapse.timeReg] = SR_incoherence(options, Y, data);
end;
