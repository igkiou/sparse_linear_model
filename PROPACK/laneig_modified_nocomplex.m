function [V,D,bnd,j,work] = laneig_modified_nocomplex(A,nin,k,sigma,options)
...
while neig < k 
  ...
  [V,T,r,anorm,ierr,w] = lanpro_modified_nocomplex(A,j,r,options,V,T,anorm);
  work= work + w;
  ...
  % Analyze T
  [D,top,bot,err] = tqlb_mex([full(diag(T))],full([0;diag(T,1)]));
  %  if err>0
