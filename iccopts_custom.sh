...
# note that cce is the 64-bit version and cc is the 32-bit version
            RPATH="-Wl,-rpath-link,$TMW_ROOT/bin/$Arch"
            CC='icc'
            CFLAGS='-DUSE_MATLAB_INTERFACE -DUSE_DOUBLE_PRECISION -D_GNU_SOURCE -fexceptions -DMKL_ILP64'
            CFLAGS="$CFLAGS -fPIC -fno-omit-frame-pointer -Wall"
            CFLAGS="$CFLAGS -I/opt/intel/mkl/10.2.5.035/include/ -I/opt/intel/Compiler/11.1/072/include/"
            CLIBS="$RPATH $MLIBS"
            CLIBS="$CLIBS -L/opt/intel/Compiler/11.1/072/lib/intel64/"
            CLIBS="$CLIBS -L/opt/intel/mkl/10.2.5.035/lib/em64t/ -Wl,--start-group -lmkl_intel_ilp64 -lmkl_intel_thread -lmkl_core -Wl,--end-group -liomp5 -lpthread -lsvml -limf"
            CLIBS="$CLIBS -lm"
			COPTIMFLAGS='-Wall'
#            COPTIMFLAGS='-fast -no-prec-sqrt -DNDEBUG -parallel -openmp -pthread'
...
#
            CXX='icpc'
            CXXFLAGS='-DUSE_MATLAB_INTERFACE -DUSE_DOUBLE_PRECISION -D_GNU_SOURCE -DMKL_ILP64'
            CXXFLAGS="$CXXFLAGS -fPIC -fno-omit-frame-pointer -Wall"
            CXXFLAGS="$CXXFLAGS -I/opt/intel/mkl/10.2.5.035/include/ -I/opt/intel/Compiler/11.1/072/include/"
            CXXLIBS="$RPATH $MLIBS"
            CXXLIBS="$CXXLIBS -L/opt/intel/Compiler/11.1/072/lib/intel64/"
            CXXLIBS="$CXXLIBS -L/opt/intel/mkl/10.2.5.035/lib/em64t/ -Wl,--start-group -lmkl_intel_ilp64 -lmkl_intel_thread -lmkl_core -Wl,--end-group -liomp5 -lpthread -lsvml -limf"
            CXXLIBS="$CXXLIBS -lm"
            CXXOPTIMFLAGS='-fast -no-prec-sqrt -DNDEBUG -parallel -openmp -pthread'
...
#
            LD="$COMPILER"
            LDEXTENSION='.mexa64'
            LDFLAGS="-pthread -shared -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE -Wl,--no-undefined -L/opt/intel/mkl/10.2.5.035/lib/em64t/libmkl_solver_ilp64.a -Wl,--start-group -lmkl_intel_ilp64 -lmkl_intel_thread -lmkl_core -Wl,--end-group -lpthread -liomp5 -lpthread -lsvml -limf"
            LDOPTIMFLAGS='-O'
            LDDEBUGFLAGS='-g'
#
            POSTLINK_CMDS=':'
