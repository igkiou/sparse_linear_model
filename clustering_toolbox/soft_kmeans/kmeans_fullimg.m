...
CIFAR_DIR='/home/igkiou/MATLAB/datasets_all/cifar-10-batches-mat/';
...
trainX = bsxfun(@minus, trainX, mean(trainX,2));
trainX = bsxfun(@rdivide, trainX, sqrt(sum(trainX .^ 2, 2)));
% trainY = bsxfun(@rdivide, bsxfun(@minus, trainY, mean(trainY,2)), sqrt(sum(trainY .^ 2, 2)));

[Vec m] = trainPCA(trainX, [], 512);
trainX = bsxfun(@minus, trainX, m);
trainX = trainX * Vec;
% trainY = bsxfun(@minus, trainY, m);
% trainY = trainY * Vec;
