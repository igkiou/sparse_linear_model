function [K, converged, X, violations] = nmmds_fpc(numPoints, fullConstraintMat,...
...
for iterRepeat = 1:numRepeats,
	...
	for iter = 1:numIters,
		...
		for iterConstraint = 1:numConstraints,
			...
			% NOTE: The following, even though incorrect, greatly increases
			% convergence. Mex-version no longer uses it.
...
% 				K = K - delta * (z*z'-y*y');
 		...
 		[K foo rankEstimate] = nuclear_psd_proximal(K, tau * delta, rankIncrement, rankEstimate);
...
function [obj deriv] = nmmds_obj_grad(K, constraintMat, betaVector)
...
% 				y = zeros(numPoints, 1); y(coord3) = 1; y(coord4) = -1;
% 				K = K - delta * (z*z'-y*y');
		obj = obj + dij - dkl - beta;
		deriv(coord1, coord1) = deriv(coord1, coord1) + 1;
		deriv(coord2, coord2) = deriv(coord2, coord2) + 1;
		deriv(coord3, coord3) = deriv(coord3, coord3) - 1;
		deriv(coord4, coord4) = deriv(coord4, coord4) - 1;
		deriv(coord1, coord2) = deriv(coord1, coord2) - 1;
		deriv(coord2, coord1) = deriv(coord2, coord1) - 1;
		deriv(coord3, coord4) = deriv(coord3, coord4) + 1;
		deriv(coord4, coord3) = deriv(coord4, coord3) + 1;
	end;
end;
