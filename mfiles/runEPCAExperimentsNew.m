...
dictParams.dictionaryNumIters = 250;
dictParams.dictionaryClearing = 0;
for iterDim = 1:numDims,
	dictParams.dictionarySize = dims(iterDim);
	D = epca_learning(trainFeaturesRCV1, [], [], dictParams);
	save(sprintf('~/MATLAB/sparse_linear_model/text_experiments/epca/data%s_vocab%s_select%s_numwords%d_dim%d, numpc%d_split%d_stochastic_custom.mat',...
		params.textDataSet, params.textVocabulary, params.textSelectionMethod, params.textNumWords, dims(iterDim), numTrainPerClass, split),...
		'params', 'trainFeatures', 'trainLabels', 'testFeatures', 'testLabels', 'D', 'dictParams');
end;
