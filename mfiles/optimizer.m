function L=optimizer(data,Responses,L,rD,dict,ReguAlpha,ReguBeta,Numiters)
%
...
if(size(Responses,1)~=rD), 
	error('Responses and L must have matching dimensions!\n');
end;
...
obj(iter) = orth_lsqr_obj_grad(vec(L), X, Y, DDt, pars.weight1, pars.weight2);
...
if(~pars.quiet),
	fprintf(['  Obj:%2.2f Delta:%2.4f max(G):%2.4f max(L):%2.4f max(O):%2.4f\n'],obj(iter),delta,max(max(abs(df))),max(max(abs(dflsqr))),max(max(abs(dforth))));
end;
