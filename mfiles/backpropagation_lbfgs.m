function [Phi, weights, bias, stdvec] = backpropagation_lbfgs(initPhi, initweights, initbias, origData, Y, stdvec)
...
% 	wXtensor = bsxfun(@times, Wt, Xtt);
	...
	[retval, xstarbest, xstarfinal, history] = ...
		lbfgs2(initPhi(:), lbfgs_options, gradphifun, [], origData, ooclabels, weightsstd, bias);
	...
	save(sprintf('sanjeev/mat_files/interimnew%d', iter), 'Phi', 'weights', 'bias', 'stdvec');
end;
