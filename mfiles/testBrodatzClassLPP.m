...
for iterClass = 1:numClassPairs,
	...
	for iterNorm = useNorm,
		for iterPatchSize = patchSize,
			...
			Vec = trainLPP(trainFeatures', trainLabels', maxDimension, 'Euclidean', 'KNN');
			...
			fprintf(fid, '\n');
			fprintf(fid, '%g ', accuracy_LPP_knn);
			fprintf(fid, '\n');
			...
			load(sprintf('brodatz_experiments/brodatz_%s_LPP%d_norm%d_patch%d_large%d_%s',...
			...
			fprintf(fid, '\n');
			fprintf(fid, '%g ', accuracy_LPP_kernel_knn);
			fprintf(fid, '\n');
			clear fea_Train_Reduced_Large fea_Test_Reduced_Large Vec
		end;
	end;
