function [D, A] = kerneldictgradient_representer(KXX, params)
...
perm_ids = randperm(numsamples);

if (isfield(params,'initdict') && ~any(isnan(params.initdict(:)))),
  if (any(size(params.initdict)==1) && all(iswhole(params.initdict(:)))),
	D = zeros(numsamples, dictsize);
    D(params.initdict(1:dictsize), 1:dictsize) = 1;
  else
    if (size(params.initdict,1)~=size(KXX,1) || size(params.initdict,2)<dictsize),
  ...
  D = zeros(numsamples, dictsize);
  D(perm_ids(1:dictsize), 1:dictsize) = 1;
end;
...
for iter = 1:iternum
  ...
  KDX = D' * KXX;
  KDD = KDX * D;
%   A = l1kernel_featuresign_mex(KXX, D, codinglambda, [], [], [], [], KDD, KDX);
  lassoparams.lambda = codinglambda;
  A = mexLasso(diag(sqrt(lvec)) * U', diag(sqrt(lvec)) * U' * D, lassoparams);
  ...
  D = l2kernel_representer_learn_basis_dual(KXX, A, 1);
...
% TODO: Implement cleardict for kernel_representer case.
