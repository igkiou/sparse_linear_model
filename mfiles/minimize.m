function [X, fX, i] = minimize(X, f, length, verbose, varargin)

% Minimize a differentiable multivariate function. 
%
% Usage: [X, fX, i] = minimize(X, f, length, verbose, P1, P2, P3, ... )
%
...
% If "verbose" is set to 1, then the objective value after each iteration
% will be printed.
%
...
% 	  disp('Negative or NaN or Inf or negative subroot.');
      x3 = x2*EXT;                                 % extrapolate maximum amount
    elseif x3 > x2*EXT                  % new point beyond extrapolation limit?
% 	  disp('New point beyond extrapolation limit.');
      x3 = x2*EXT;                                 % extrapolate maximum amount
    elseif x3 < x2+INT*(x2-x1)         % new point too close to previous point?
% 	  disp('New point too close to previous point.');
      x3 = x2+INT*(x2-x1);
	end
...
% 	  gkiou = gkiou + 1;
% 	  disp(sprintf('Iter %d, second while iter %d\n', i, gkiou));
