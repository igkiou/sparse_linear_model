...
if (printinfo),
  disp(sprintf('Starting learning exponential sparsifying dictionary.\n')); %#ok
end;
...
%   D = l2exp_learn_basis_gradient_projection_backtracking(X, A, D, family);
  D = l2exp_learn_basis_gradient_backtracking_mex(Xblock, A, D, family);
  ...
  if (printinfo),
	  if (errorinfo),
		  disp(sprintf('basisObj = %g, reconObj = %g\n', basisObj, reconObj)); %#ok
	  end;
