function [astar xstar fstar gstar varargout] = lineSearchWolfeProjected(x0, f0, g0, s0, a1, amax, c1, c2, maxiter, f, fproj, varargin)
...
while 1
  ...
  [fi gi ] = feval(f, fproj(xstar), varargin{:});
  linegradi = gi(:)'*s0(:);  
  if fi > (f0+c1*ai*linegrad0) || ( fi >= fi_1 && i > 1)
    ...
    [astar xstar fstar gstar ] = zoom(ai_1, ai, x0, f0, g0, s0, c1, c2, ...
                                        linegrad0, fi_1,  linegradi_1,fi, linegradi, f, fproj, varargin{:});
	xstar = fproj(xstar);
    return;
  end
  ...
  if linegradi >=0
    ...
    [astar xstar fstar gstar ] = zoom(ai, ai_1, x0, f0, g0, s0, c1, c2, ...
                                        linegrad0, fi, linegradi, fi_1, linegradi_1, f, fproj, varargin{:});
	xstar = fproj(xstar);
    return;
  end
  i = i+1;
...
function [astar xstar fstar gstar varargout] = zoom(alo, ahi, x0, f0, g0, ...
                                                    s0, c1, c2, linegrad0, ...
                                                    falo,galo, fhi, ghi,  f, fproj, varargin)
echo = false;
i=0;
varargout(1) = {[]};
while 1
  ...
  xstar = x0+aj*s0;
  [fj, gj ] = feval(f, fproj(xstar), varargin{:});
