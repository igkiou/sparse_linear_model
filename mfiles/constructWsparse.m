...
%               'Euclidean'    - If nodes i and j are connected, put weight
%                                norm(x_i - x_j). This weight mode can only
%                                be used under 'Euclidean' metric.
...
switch lower(options.WeightMode)
    ...
    case {lower('Euclidean')}
        if ~strcmpi(options.Metric,'Euclidean')
            warning('''Euclidean'' WeightMode should be used under ''Euclidean'' Metric!');
            options.Metric = 'Euclidean';
        end
