function [obj deriv derivlsqr derivorth] = orth_lsqr_obj_grad(Phi, X, Y, DDt, alphaReg, betaReg)
...
if (betaReg > 0),
	obj = obj + betaReg * norm(Phi * DDt * Phi', 'fro') ^ 2;
end;
...
if (nargout >= 2),
	...
	if (betaReg > 0),
		PhiDDt = Phi * DDt;
		deriv = deriv + betaReg * 4 * PhiDDt * Phi' * PhiDDt;
	end;
	derivorth = deriv - derivlsqr;
	deriv = deriv(:);
end;
