function [sift_arr label_arr] = getSiftBatch(I, GT, grid_x, grid_y, patch_size, sigma_edge)
...
for i=1:num_patches
    ...
    sample_x_t = sample_x * r + cx(i);
    sample_y_t = sample_y * r + cy(i);
...
%     weights = weights ./ repmat(tmp, [1 num_samples]);
	...
	label_arr(i) = (GT(ceil(cy(i)), ceil(cx(i))) > 0);
