function [f, df] = multisquaredhinge_kernel_obj_grad(kernelMatrix, W, X, Y, lambda, classLabels, useregularizer)
...
for iterT = 1:numTasks,
	...
	Ytemp(Ytemp == 0) = -1;
	[fTemp, dfTemp] = squaredhinge_kernel_obj_grad(kernelMatrix, W(:, iterT), X, Ytemp, lambda, useregularizer);
	f = f + fTemp;
	df = df + dfTemp;
end;
